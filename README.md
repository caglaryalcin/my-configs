## My browser extensions backup

### uBlock
```
curl -o $env:userprofile\Desktop\uBlock_backup.txt https://raw.githubusercontent.com/caglaryalcin/browser-config/main/ublock.txt
```

### uBlacklist
```
https://raw.githubusercontent.com/caglaryalcin/browser-config/main/uBlacklist.txt
```

### Dark Reader
```
curl -o $env:userprofile\Desktop\dark-reader_backup.json https://raw.githubusercontent.com/caglaryalcin/browser-config/main/darkreader.json
```
### My bookmarks
```
curl -o $env:userprofile\Desktop\dark-reader_backup.json https://raw.githubusercontent.com/caglaryalcin/browser-config/main/bookmarks.json
```
